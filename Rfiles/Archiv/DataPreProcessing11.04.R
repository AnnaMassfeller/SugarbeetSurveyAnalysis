...
g.Intentions<-ggplot(freq.intentions_long,aes(x= factor(Intention, level = c("no Intention", "low","middle","high","technique in use")),value,fill=variable))+
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1),legend.position = "bottom")+
  labs(x = "Level of Intention", y = "Count")+
  scale_fill_brewer(name = "Type of mechanical weeding", labels = c("traditional", "modern", "autonomous"),palette = "Spectral")
g.Intentions
...
summary(mInt1<- multinom(q6_col1 ~ q3_info +  NrFields + age_b + farmsize_b + AES_b, 
                   data = FullSample))

summary(mInt2<- multinom(q6_col2 ~ q3_info +  NrFields + age_b + farmsize_b + AES_b, 
                   data = FullSample))

summary(mInt3<- multinom(q6_col3 ~ q3_info +  NrFields + age_b + farmsize_b + AES_b, 
                   data = FullSample))
