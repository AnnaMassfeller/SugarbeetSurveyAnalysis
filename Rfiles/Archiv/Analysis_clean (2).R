...
#check for differecnes between adopters & non-adopters
t.test(AdoptersIV$Peers_cont, NonAdoptersIV$Peers_cont)
t.test(AdoptersIV$Fields_cont, NonAdoptersIV$Fields_cont)
t.test(AdoptersIV$meanDist, NonAdoptersIV$meanDist)
...
SampleIV$meanDist <- as.numeric(SampleIV$meanDist)
...
p.hist3 <- ggplot(SampleIV, aes(meanDist))+
  geom_histogram(binwidth = 5)+
  theme_bw()+
  scale_x_continuous(limits = c(0, 50))+
  scale_y_continuous(limits = c(0, 60))+
...
SampleIV<- SampleIV
SampleIV$meanDist<-ifelse(SampleIV$meanDist == "200", "NA",SampleIV$meanDist)
SampleIV$meanDist <- as.numeric(SampleIV$meanDist)
SampleIV_map <- SampleIV[!is.na(SampleIV$q5_other),]
SampleIV_mc <- SampleIV[!is.na(SampleIV$q5_alt_dist),]
SampleIV$map <- ifelse(!is.na(SampleIV$q5_other),1,0)
...
m.dist_map <-glm(q1_adopt ~ info_b 
                 + meanDist+
                   ...
                   # advisory,
                   Fabrikstandort, 
             data = SampleIV_map, family = binomial("probit"))
...
p.hist3_map <- ggplot(SampleIV_map, aes(meanDist))+
...
m.dist_mc <-glm(q1_adopt ~ info_b 
                 ...
                 data = SampleIV_mc, family = binomial("probit"))
...
p.hist3_mc <- ggplot(SampleIV_mc, aes(meanDist))+
  geom_histogram(binwidth = 2)+
  theme_bw()+
...
p.hist3_mc_map <- ggplot(SampleIV, aes(meanDist, fill = map))+
  geom_histogram(binwidth = 2)+
  theme_bw()+
 # labs(title = "multiple choice")+
  scale_x_continuous(limits = c(0, 50))+
