...
reg_double_fabric <- glm(q1_adopt ~ .,data=dfDouble, family = "binomial")
summary(reg_double_fabric)
df.Lasso_selected_fabric <- reg_double_fabric[["model"]] 
vtable(df.Lasso_selected_fabric, values = TRUE, missing = TRUE)
#marginal effects
m.Lasso_select_mfx_fabric <-probitmfx(reg_double_fabric, data = dfDouble)
m.Lasso_select_mfx_fabric
coefs_fabric <-  m.Lasso_select_mfx_fabric[["mfxest"]]
```
Compare that to the result that we would obtained for OLS on all variables
```{r}
dat$q1_adopt <- as.factor(dat$q1_adopt)
reg_Full_fabric <- glm(q1_adopt ~ .,data=dat,family = "binomial")
summary(reg_Full_fabric)
m.reg_Full_fabric_mfx <-probitmfx(reg_Full_fabric, data = dat)
m.reg_Full_fabric_mfx

plot_summs(m.reg_Full_mfx,m.reg_Full_fabric_mfx,m.Lasso_select_mfx,m.Lasso_select_mfx_fabric,m.Full.comp_mfx3, 
           ...
           colors = c("Paired"),
           model.names = c("Full Verband", "Full Fabrikstanddort", "Lasso Verband", "Lasso Fabrikstandort", "Preregistration"),
